...
- **The flow is one-way, and it is lossy in both directions.** `blog-drafts` → Ghost is manual copy-paste; Ghost → `blog-drafts` does not exist. An edit made in Ghost's editor after publication appears nowhere in git, so a fix applied to a draft does not reach the live post and a fix applied to the live post does not reach the draft. The two diverge silently, and nothing detects it.

  **This is a caveat on every count in this document.** All the drift figures below were measured against `blog-drafts`, which describes the drafts and not the blog. The published posts could match, or not, in either direction — there is no way to tell without comparing them by hand. Treat the numbers as "what authors wrote," not "what readers see."
...
- **`blog-drafts` is the review surface, which is why it persists.** All editorial review of posts happens in GitHub, on the markdown. That is the reason the repo exists despite not being the source of truth for what is published, and it is why a pipeline that skips Ghost's editor — which would be Mara's preference — is not on the table. **Do not propose one.**
  ...
  **The consequence is a genuine tension in this problem.** The document that gets reviewed and the document that gets published are different artifacts, and every block moved out of post source widens that gap. A CTA that becomes a Ghost Snippet is no longer visible to a GitHub reviewer — the post under review is missing its ending.
  ...
  For boilerplate that is identical every time, widening the gap is probably *desirable*: reviewers stop scrolling past three blocks they are not reviewing, and the blocks stop drifting. For the NSF acknowledgment it is less clear, since it is conditional and carries required language a reviewer might reasonably want to see in context. **The design session should say, per block, whether it belongs in the reviewed document or in the published one** — that is the actual decision, and it is not the same question for all three.

  Worth noting the current fix pattern, since it is the only one that works today: edit in Ghost first, where it reaches readers, then back-port to `blog-drafts` so the review surface stays honest. Two deliberate steps in that order.
...
**Author bio — the best-organised of the three, and the theme ignores it entirely.** There is a canonical `author_blurb.html` per author — nine of them, one in each author directory, not one per post. Each holds a single `<p class="dev-theme-author-blurb">` with an italic sentence. Twelve posts across four authors paste it in, and **eleven of the twelve match their author's canonical file byte-for-byte.** The one exception is `gunj-joshi/bringing-complex-computations-to-browsers`, where the post says "core developer of stdlib, a JavaScript library for numerical and scientific computing" against a canonical "core contributor to stdlib".

So this block already has both a per-author source of truth and a stable class, and **the theme styles that class nowhere** — it renders as a plain italic paragraph. The convention is established, applied, and unused. Any design here can key off markup authors already write.

Two things worth knowing about the files: three of the nine (`aman-bhansali`, `jaysukh-makvana`, `karan-anand`) still read "is a TODO and", so the canonical set is partly unwritten; and the per-author-directory placement means an author's bio is versioned once rather than per post, which is the right shape for a transclusion mechanism if one is ever built.
