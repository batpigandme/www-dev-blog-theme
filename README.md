...
**What's actually verified, and what isn't.** `@tryghost/kg-markdown-html-renderer` — the package Ghost itself uses to render a Markdown card's content — passes `<aside>` markup through completely unmodified (`class`, `role`, `aria-label`, `data-placement` all intact; nothing in Ghost's pipeline sanitizes or strips it). That's real, but it only proves the *render* function is safe once `<aside>` markup is correctly sitting inside an HTML card. It does **not** prove that pasting or typing `<aside>` inline, as part of surrounding prose, reliably produces that HTML card in the first place — a real, hands-on test of the equivalent `<figure>` pattern (2026-07-14, against a local Ghost 6.51.0 instance) found that a genuine OS-level paste merges inline raw HTML directly into the surrounding paragraph's running text instead, not into any kind of card. **Always add an aside as its own deliberate HTML card** (`/html` in the Koenig editor) — don't write `<aside>` inline in a Markdown card or paste it as part of a larger block and expect it to convert.

**Also not yet verified**: whether footnotes elsewhere in the same post still resolve correctly when an aside's HTML card sits between paste-derived paragraphs. The footnote-repair test coverage so far is Lexical-paste-stripped content and refs/defs split across two Markdown cards — not paste-derived content interrupted by an HTML card. Treat this as untested, not assumed-safe, until it's specifically checked.
