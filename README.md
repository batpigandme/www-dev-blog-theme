...
By default, footnotes are **endnotes only** — a plain numbered list at the end of the post, no margin mirroring. To also mirror each note into the right margin beside its reference (on viewports at least 1120px wide), tag the post `#sidenotes` in Ghost. `#`-prefixed tags are internal — hidden from readers, never shown on tag pages — but Ghost's built-in `post_class` helper still reflects them as a class on `<article>` (`tag-hash-sidenotes`), which `footnotes.js` checks; no template change is needed to use it. Margin copies are `aria-hidden` visual clones (non-tabbable, `id`-stripped); the endnotes remain the canonical, interactive copy at every viewport width, tagged or not.
